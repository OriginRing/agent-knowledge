...
from models.admin_models import AdminResource, AdminVersion, AdminDebugRun, AdminSkillLease
...
class AdminStorageTest(unittest.TestCase):
    def setUp(self):
        ...
        for table in (AdminResource.__table__, AdminVersion.__table__, AdminDebugRun.__table__, AdminSkillLease.__table__, AgentList.__table__, User.__table__):
    ...
    def test_publish_draft_isolation_offline_and_conflict(self):
        ...
        payload = {'name': '测试智能体', 'model_type': 'ollama', 'model_name': 'test', 'slot': [],
        ...
        changed = {**agent['draft'], 'model_name': 'next'}
        ...
        self.assertEqual(admin.published_config(code)['model_name'], 'next')
    ...
    def test_agent_workflow_deletion_rules_and_usage(self):
        flow = admin.save('workflows', graph())
        flow = admin.publish('workflows', flow['id'], flow['revision'])
        agent = admin.save('agents', {'name': '引用测试', 'model_type': 'ollama', 'model_name': 'test',
    ...
    def test_latest_skill_and_workflow_require_agent_republish(self):
        ...
        agent = admin.save('agents', {'name': 'latest', 'model_type': 'ollama', 'model_name': 'test', 'workflowId': workflow['id']})
        agent = admin.publish('agents', agent['id'], agent['revision'])
